# analysis_newdata_filter_maxpop.R
...
if(USE_NEW_DATA) {
  headers = read.csv('spaltennr_labels_NEWDATA.csv', sep=',', header = F, stringsAsFactors = F)
} else {
...
# Flächen
std.plot(d = data.flaechen, "gebfreifl.rel", plot.title = "Anteil Gebäude- und Freifläche an Gesamtfläche", xlab = "Flächenanteil",  percent = T, upper_break = 0.12)
std.plot(d = data.flaechen, "betrfl.rel", plot.title = "Anteil Betriebsfläche an Gesamtfläche", xlab = "Flächenanteil",  percent = T, upper_break = 0.05, x.ticks.n = 4)
std.plot(d = data.flaechen, "erholfl.rel", plot.title = "Anteil Erholungsfläche an Gesamtfläche", xlab = "Flächenanteil",  percent = T, upper_break = 0.04, x.ticks.n = 4)
...
# Schüler
std.plot(d = data.schueler, "schueler.insg.rel", "Schüler", xlab = "Anteil an Bevölkerung", percent = T, upper_break = 0.2)

# Sozialhilfeempfänger
std.plot(d = data.sozhilfe, "sozialhilfe_empf.insg.rel", "Sozialhilfeempfänger (SGB XII Kap. 3-9)", xlab = "Anteil an Bevölkerung", upper_break = 0.1, percent = T)

# Bevölkerung
std.plot(COLLABELS$bevoelkerung.insgesamt, plot.title = "Bevölkerung 2017", xlab = "Bevölkerung", upper_break = 5000, x.ticks.n = 5)
...
std.plot(COLLABELS$bevoelkerung.bewegung.zugezogene, "Bevölkerungsbewegung: Zugezogene (2017)", xlab = "Anzahl Zugezogener", upper_break = 450, x.ticks.n = 5)
std.plot(COLLABELS$bevoelkerung.bewegung.fortgezogene, "Bevölkerungsbewegung: Fortgezogene (2017)", xlab = "Anzahl Fortgezogener", upper_break = 450, x.ticks.n = 1)
std.plot(COLLABELS$bevoelkerung.bewegung.wanderungsgewinn, "Bevölkerungsbew.: Wanderungsgewinn (2017)", xlab = "Wanderungsgewinn", lower_break = -50, upper_break = 110, x.ticks.n = 6)

# Beschäftigte
std.plot(COLLABELS$erwerb.sozialverspfl_beschaeftigte_am_arbeitsort.insgesamt, "Beschäftigte am Arbeitsort", xlab = "Anzahl Beschäftigter", upper_break = 2500, x.ticks.n = 6)
std.plot(COLLABELS$erwerb.sozialverspfl_beschaeftigte_am_wohnort.darunter_auspendler.prozent, "Beschäftigte am Wohnort, darunter Auspendler", xlab = "Anteil Auspendler in %", percent = F, x.ticks.n = 2)
std.plot(COLLABELS$erwerb.sozialverspfl_beschaeftigte_am_arbeitsort.darunter_einpendler.prozent, "Beschäftigte am Arbeitsort, darunter Einpendler", xlab = "Anteil Einpendler in %", percent = F, x.ticks.n = 3)
std.plot(COLLABELS$erwerb.pendlersaldo, "Pendlersaldo", xlab = "Pendlersaldo", lower_break = -1500, upper_break = 1500)

# Landwirtschaft
std.plot(COLLABELS$landforst.betriebe_von_flaeche_ha.insgesamt, "Land- und forstwirtschaftliche Betriebe", xlab = "Anzahl Betriebe", x.ticks.n = 6)

# Wasser pro Kopf Verbrauch
std.plot(COLLABELS$umwelt.wasser_pro_kopf_verbrauch, "Wasserverbrauch", xlab = "Liter pro Person", lower_break = 80, upper_break = 180, x.ticks.n = 2)

# Wohnungen
std.plot(COLLABELS$bauwohn.baugenehmigungen.wohnungen, "Erteilte Baugenehmigungen: Wohnungen (2017)", xlab = "Anzahl", upper_break = 60)
std.plot(COLLABELS$bauwohn.bestand_wohnungen.insgesamt, "Wohnungsbestand", xlab = "Anzahl", upper_break = 2300, x.ticks.n = 1)

# Finanzen
std.plot(COLLABELS$lohneinksteuer.gesamtbetrag_einkuenfte.insgesamt, "Lohn- und Eink.-St.: Gesamtbetrag der Einkünfte (2014)", xlab = "in 1000€", upper_break = 120000, x.ticks.n = 6)
std.plot(COLLABELS$lohneinksteuer.gesamtbetrag_einkuenfte.je_steuerpfl, "Lohn- und Eink.-St.: Einkünfte je Steuerpflichtiger (2014)", xlab = "in 1000€", upper_break = 80000, x.ticks.n = 2)
std.plot(COLLABELS$lohneinksteuer.bruttolohn.je_arbeitnehmer, "Lohn- und Eink.-St.: Bruttolohn je Arbeitnehmer (2014)", xlab = "in €", upper_break = 42000, x.ticks.n = 1)
std.plot(COLLABELS$kommunale_finanzen.gemeindesteuereinnahmen.insgesamt, "Gemeindesteuereinnahmen: insgesamt", xlab = "in 1000€", lower_break = 0, upper_break = 8000, x.ticks.n = 4)
std.plot(COLLABELS$kommunale_finanzen.steuereinnahmen_insgesamt, "Steuereinnahmen: insgesamt", xlab = "in € je Einwohner", lower_break = 0, upper_break = 2000, x.ticks.n = 2)
...
std.plot(COLLABELS$kommunale_finanzen.steuereinnahmekraft.je_einwohner, "Steuereinnahmekraft", xlab = "in € je Einwohner", upper_break = 3000, x.ticks.n = 3)
...
std.plot(COLLABELS$kommunale_finanzen.gemeindesteuereinnahmen.darunter.grundsteuer.b, plot.title = "Gemeindesteuereinnahmen: Grundsteuer B", xlab = "in 1000€", upper_break = 800, x.ticks.n = 2)
